...
for ($i=0;$i<$rows_cnt;$i++){
	...
	if(time()<$end_time+$OJ_RANK_LOCK_DELAY&&$lock<strtotime($row['in_date']))
        	   ...
        	   $U[$user_cnt]->Add($row['num'],strtotime($row['in_date'])-$start_time,$row['pass_rate'],$row['result'],$row['ladder_score']);
}
usort($U,"s_cmp");
...
$finished=0;
if($OJ_MEMCACHE){
	$sql="SELECT finished FROM contest WHERE `contest_id`='$cid'";
        $result = mysql_query_cache($sql);
        if($result) $finished=$result[0][0];
        else $finished=1;
}else{
	$sql="SELECT finished FROM contest WHERE `contest_id`=?";
        $result = pdo_query($sql,$cid);
 	if($result) $finished=$result[0][0];
        else $finished=1;      
}
...
/////////////////////////Template
require("template/".$OJ_TEMPLATE."/contestrank-ladder.php");
/////////////////////////Common foot
